...
static int qpnp_wled_set_max_current(struct qpnp_led_data *led,
										int max_cur)
{
	int rc, i;
	u8 max_cur_val, val;
	int max_cur_changed = 0;

	rc = spmi_ext_register_readl(led->spmi_dev->ctrl,
		led->spmi_dev->sid, WLED_FULL_SCALE_REG(led->base, 0),
		&max_cur_val, 1);
	if (rc) {
		dev_err(&led->spmi_dev->dev,
			"%s: WLED read max current reg failed(%d)\n", __func__, rc);
		goto error;
	}
	dev_dbg(&led->spmi_dev->dev,
		"%s: read max_current %dmA!!", __func__, max_cur_val);
	if (max_cur_val != max_cur) {
		dev_dbg(&led->spmi_dev->dev,
			"%s: Set max_current %dmA!!", __func__, max_cur);
		...
		if (rc) {
			dev_err(&led->spmi_dev->dev,
				"%s: WLED write ctrl reg failed(%d)\n", __func__, rc);
			goto error;
		}
		...
		for (i = 0; i < led->wled_cfg->num_strings; i++) {
			rc = qpnp_led_masked_write(led,
				...
				WLED_MAX_CURR_MASK, max_cur);
			if (rc) {
				dev_err(&led->spmi_dev->dev,
					"%s: Write max current failure (%d)\n",
					__func__, rc);
				goto error;
			}
		}
...
static int qpnp_wled_set(struct qpnp_led_data *led)
{
	...
	if (level == 0) {
		...
		max_cur_changed = qpnp_wled_set_max_current(led, 14);

		/* brightness mapping :
		   set level based on 14mA from 19mA base level*/
		dev_dbg(&led->spmi_dev->dev,
				"input level is %d", level);
		level = level*level*49/10000 + 85;
		dev_dbg(&led->spmi_dev->dev,
				"shifted level is %d", level);
		...
		max_cur_changed = qpnp_wled_set_max_current(led, led->max_current);
#endif
		val = WLED_BOOST_ON;
		rc = spmi_ext_register_writel(led->spmi_dev->ctrl,
			led->spmi_dev->sid, WLED_MOD_CTRL_REG(led->base),
