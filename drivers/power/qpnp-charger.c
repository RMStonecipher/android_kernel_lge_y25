...
#elif defined (CONFIG_MACH_MSM8926_E2_MPCS_US) || defined (CONFIG_MACH_MSM8926_E2_VTR_CA) || defined(CONFIG_MACH_MSM8926_E2JPS_JP)
...
#ifdef CONFIG_MACH_MAM8926_VFP_KR
...
{
	struct qpnp_chg_chip *chip = _chip;
#ifdef CONFIG_MACH_MSM8926_E8LTE_KR
	...
	if(!is_factory_cable()){
		...
		pr_err("======= [TRKL CHG IRQ] BOOT Mode = %d DCP = %d, SDP = %d ========\n",boot_mode,chip->ac_online, qpnp_chg_is_usb_chg_plugged_in(chip));
		if(boot_mode == LGE_BOOT_MODE_QEM_56K) {
		//In case of MiniOS booting with Normal cable or without cable
#ifdef CONFIG_LGE_PM_CHARGING_EXTERNAL_OVP
			// DCP Cable
			if (chip->ac_online && qpnp_chg_is_usb_chg_plugged_in(chip))
			{
				pr_err("======= [TRKL CHG IRQ] DCP CABLE - Ext. OVP is HIGH =============\n");
				lge_set_chg_path_to_external();
			}
			// SDP Cable
			else if (!chip->ac_online && qpnp_chg_is_usb_chg_plugged_in(chip))
			{
				pr_err("======= [TRKL CHG IRQ] SDP CABLE - Ext. OVP is LOW =============\n");
				lge_set_chg_path_to_internal();
			}
#endif
		}
...
#ifdef CONFIG_MACH_MSM8926_E8LTE_KR
			if( get_prop_batt_present(chip) && (qpnp_chg_usb_iusbmax_get(chip) < 1500)) {
				pr_info("Factory cable is connected (130K) SET 1500mA, instead of 100mA \n");
