	...
	else
	{
		...
		else
		{
			if( pLpwgSetting->mode == 0 ) {
				...
				TOUCH_ERR("State was forcely changed to OFF\n");
				nextState = STATE_OFF;
			}
		}
	}
...
static ssize_t store_lpwg_notify(struct i2c_client *client, const char *buf, size_t count)
{
	...
	if( ( lpwgCmd != LPWG_CMD_MODE ) && ( lpwgCmd != LPWG_CMD_UPDATE_ALL ) && ( lpwgCmd != LPWG_CMD_CALL ) ) {
...
static void touch_early_suspend(struct early_suspend *h)
{
	TouchDriverData *pDriverData =
		container_of(h, TouchDriverData, early_suspend);
	...
	if( TouchGetBootMode() == BOOT_MINIOS ) /* MFTS */
	{
		/* disable interrupt */
		
		/* turn off the power of touch and display */
	}
...
static void touch_late_resume(struct early_suspend *h)
{
	...
	if( TouchGetBootMode() == BOOT_MINIOS ) /* MFTS */
	{
		/* turn on the power of touch and display */
	...
	unsigned long event, void *data)
{
	...
	TouchDriverData *pDriverData =
		container_of(self, TouchDriverData, fb_notif);
	...
	if (evdata && evdata->data)
	{
		...
		{
			if (*blank == FB_BLANK_UNBLANK) // Resume ( will be called after LCD On )
			{
				TOUCH_FUNC_EXT("Resume\n");

				/* to avoid un-expected two times of call happened during the boot */
				if( pDriverData->isSuspend == TOUCH_FALSE ) {
					TOUCH_WARN("Resume on Resume\n");
					mutex_unlock(pMutexTouch);
					return TOUCH_SUCCESS;
			...
			else if (*blank == FB_BLANK_POWERDOWN) // Suspend ( will be called after LCD Off )
			{
		...
		{
			if(*blank == FB_BLANK_UNBLANK) // Early Resume ( will be called before LCD On )
			{
				TOUCH_FUNC_EXT("Early Resume\n");
				
				/* to avoid un-expected two times of call happened during the boot */
				if( isEarlySuspend == TOUCH_FALSE ) {
					TOUCH_WARN("Early Resume on Early Resume\n");
					mutex_unlock(pMutexTouch);
					return TOUCH_SUCCESS;
				} else {
			...
			else if (*blank == FB_BLANK_POWERDOWN) // Early Suspend ( will be called before LCD Off )
			{
				...
				if( isEarlySuspend == TOUCH_TRUE ) {
					TOUCH_WARN("Early Suspend on Early Suspend\n");
					mutex_unlock(pMutexTouch);
					return TOUCH_SUCCESS;
				} else {
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* do something for off-charging */
	if( TouchGetBootMode() == BOOT_OFF_CHARGING )
	{
		#if defined ( TOUCH_DEVICE_S3320 )
		/* JDI In-cell : we can't set to off because of LCD operation */
		pDriverData->nextState = STATE_NORMAL;
		#else
