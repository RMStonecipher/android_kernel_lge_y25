...
#if defined(CONFIG_SENSOR_AUTO_DETECT_ACC)
static int  k3dh_local_init(void);
static int  k3dh_local_remove(void);

static struct sensor_init_info k3dh_init_info = {
		.name = "K3DH",
		.init = k3dh_local_init,
		.uninit = k3dh_local_remove,
};
#endif
...
static ssize_t k3dh_delay_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if(data <= 200 && data >=10)
		k3dh->pdata->poll_interval = data;//Put polling delay data from HAL into poll_interval to use work queue delay.
...
#if defined(CONFIG_SENSOR_AUTO_DETECT_ACC)
	pr_info("%s accelerometer k3dh_local_init\n", K3DH_ACC_DEV_NAME);
	lge_sensor_acc_add(&k3dh_init_info);
	return 0;
#else
	pr_info("%s accelerometer k3dh_acc_init\n", K3DH_ACC_DEV_NAME);
	return i2c_add_driver(&k3dh_acc_driver);
#endif
}
