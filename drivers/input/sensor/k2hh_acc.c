...
#ifdef K2HH_ACCEL_CALIBRATION
#define K2HH_SHAKING_DETECT_THRESHOLD 200
 ...
 */
struct k2hh_acc_data *k2hh_acc_misc_data;

#if defined(CONFIG_SENSOR_AUTO_DETECT_ACC)
static int  k2hh_local_init(void);
static int  k2hh_local_remove(void);

static struct sensor_init_info k2hh_init_info = {
		.name = "K2HH",
...
static int k2hh_acc_i2c_read(struct k2hh_acc_data *acc, u8 *buf, int len)
{
	int err;
	int tries = 0;
...
static int k2hh_acc_i2c_write(struct k2hh_acc_data *acc, u8 *buf, int len)
{
	int err;
	int tries = 0;
...
static int k2hh_acc_register_write(struct k2hh_acc_data *acc, u8 *buf,
...
static int k2hh_acc_register_read(struct k2hh_acc_data *acc, u8 *buf,
		u8 reg_address)
{
...
static int k2hh_acc_hw_init(struct k2hh_acc_data *acc)
{
	...
	buf[0] = CTRL_REG4;  //Enable multiple byte register access
	buf[1] = acc->resume_state[RES_CTRL_REG4];
	err = k2hh_acc_i2c_write(acc, buf, 1);
	if (err < 0)
		goto error1;
	...
	err = k2hh_acc_i2c_write(acc, buf, 2);
	if (err < 0)
		goto error1;
	buf[0] = INT_CFG1;
	...
	err = k2hh_acc_i2c_write(acc, buf, 5);
	if (err < 0)
		goto error1;
...
{
	int err;
	u8 buf[2] = { CTRL_REG1, K2HH_ACC_PM_OFF };
...
static int k2hh_acc_device_power_on(struct k2hh_acc_data *acc)
{
	...
	pr_info("%s: end of k2hh_acc_hw_init()\n", K2HH_ACC_DEV_NAME);
...
#endif
int k2hh_acc_update_g_range(struct k2hh_acc_data *acc, u8 new_g_range)
{
	int err;
	...
	u8 init_val;
	u8 new_val;
	u8 mask = K2HH_ACC_FS_MASK | IF_ADD_INC;
...
int k2hh_acc_update_odr(struct k2hh_acc_data *acc, int poll_interval_ms)
{
	int err = -1;
	int i;
	u8 config[2];
	...
	for (i = ARRAY_SIZE(k2hh_acc_odr_table) - 1; i >= 0; i--) {
		if (k2hh_acc_odr_table[i].cutoff_ms <= poll_interval_ms || i == 0)
			break;
	}
	config[1] = k2hh_acc_odr_table[i].mask;

	config[1] |= K2HH_ACC_ENABLE_ALL_AXES;

	/* If device is currently enabled, we need to write new
	 *  configuration out to it */
	if (atomic_read(&acc->enabled)) {
		config[0] = CTRL_REG1;
		if(acc->resume_state[RES_CTRL_REG1] & K2HH_HR_MSK) //If previous mode is HR remain in HR
			config[1] |= K2HH_HR_MSK;
	...
	#if DEBUG
		pr_info("%s k2hh_acc_update_odr    mask : %d\n",
			K2HH_ACC_DEV_NAME, k2hh_acc_odr_table[i].mask);
	#endif
...
int k2hh_acc_get_odr(struct k2hh_acc_data *acc, unsigned char *odr)
{
	int err = -1;
	unsigned char data[2] = { 0, };

	err = k2hh_acc_register_read(acc, data, K2HH_ODR_REG);
	if (err < 0)
		return err;
...
static int k2hh_acc_register_update(struct k2hh_acc_data *acc, u8 *buf,
		u8 reg_address, u8 mask, u8 new_bit_values)
{
	int err = -1;
	u8 init_val;
	u8 updated_val;
	err = k2hh_acc_register_read(acc, buf, reg_address);
	if (!(err < 0)) {
		init_val = buf[1];
		updated_val = ((mask & new_bit_values) | ((~mask) & init_val));
		err = k2hh_acc_register_write(acc, buf, reg_address,
				updated_val);
	}
	return err;
}
...
static int k2hh_store_reserved_reg(struct device *dev)
{
	...
	err = k2hh_acc_i2c_read(k2hh,rsvd_l,sizeof(rsvd_l));
	if (err < 0)
		return err;

	rsvd_m[0] = (K2HH_I2C_AUTO_INCREMENT | 0x08 );
	err = k2hh_acc_i2c_read(k2hh,rsvd_m,sizeof(rsvd_m));
	if (err < 0)
		return err;

	rsvd_h[0] = (K2HH_I2C_AUTO_INCREMENT | 0x10 );
	err = k2hh_acc_i2c_read(k2hh,rsvd_h, sizeof(rsvd_h));
	if (err < 0)
		return err;
...
static void k2hh_acc_report_values(struct k2hh_acc_data *acc, int *xyz)
{
...
static int k2hh_acc_misc_open(struct inode *inode, struct file *file)
{
	int err;
	...
	file->private_data = k2hh_acc_misc_data;
...
static int k2hh_acc_get_selftest(struct k2hh_acc_data *stat, char *buf)
{
...
/*	s16 tmp = 0; */
	...
	k2hh_acc_disable(stat);
	k2hh_acc_device_power_on(stat);

	x[0] = CTRL_REG1;
	x[1] = 0xC7;
	k2hh_acc_i2c_write(stat, x, 1);

	x[0] = (K2HH_I2C_AUTO_INCREMENT | CTRL_REG4);
	x[1] = 0x04;
	x[2] = 0x00;
	x[3] = 0x00;
	k2hh_acc_i2c_write(stat, x, 3);

	mdelay(80);

	x[0] = (K2HH_I2C_AUTO_INCREMENT | AXISDATA_REG);
	k2hh_acc_i2c_read(stat, x, 6);

	for (i = 0; i < 5; i++) {
		while (1) {
			x[0] = 0x27; /*status_reg */
			val = k2hh_acc_i2c_read(stat, x, 1);
			if (val < 0) {
       ...
       /* 16 bit resolution 1LSB=0.061mg */
		...
		pr_info("%s-NO_ST(%d): %d, %d, %d\n",
			 K2HH_ACC_DEV_NAME, i, NO_ST[0], NO_ST[1], NO_ST[2]);
#endif
	}
...
#if DEBUG
	pr_info("%s:AVE_NO_ST: %d, %d, %d\n", K2HH_ACC_DEV_NAME, NO_ST[0], NO_ST[1], NO_ST[2]);
#endif
	...
	for (i = 0; i < 5; i++) {
		...
		pr_info("%s-ST(%d): %d, %d, %d\n",
			K2HH_ACC_DEV_NAME, i, ST[0], ST[1], ST[2]);
#endif
	...
	ST[2] /= 5;
		...
		pr_info("%s:AVE_ST: %d, %d, %d\n",
			K2HH_ACC_DEV_NAME, ST[0], ST[1], ST[2]);
#endif
			...
			if (i < 2) {
				if (abs(NO_ST_ZOFF[i]) > TESTLIMIT_XY) {
					pr_info("%s: NO_ST[%d]: Out of ZOffset!! (%d)\n",
						K2HH_ACC_DEV_NAME, i, NO_ST_ZOFF[i]);
					val = -1;
				}
			} else {
	...
	if (val) {
		pr_info("%s-%d: Self Test: OK!! (%d, %d, %d) ||| (%d, %d, %d)\n",
			K2HH_ACC_DEV_NAME, val, ST[0], ST[1], ST[2],
...
ST_EXIT:
	...
	k2hh_acc_device_power_off(stat);

	if (en_state)
		k2hh_acc_enable(stat);

	return val;
}

static int k2hh_acc_get_mode(struct i2c_client *client, unsigned char *mode)
{
	int err = 0;
	unsigned char data[2] = { 0, };
	...
	struct k2hh_acc_data *k2hh = i2c_get_clientdata(client);
	LPen = 0;
    err = k2hh_acc_register_read(k2hh, data, K2HH_HR_REG);
    if (err < 0)
	       return err;
	...
	pr_info("k2hh_acc_get_mode, LPen : %d, HR : %d\n",LPen,HR);

	return err;
}

static int k2hh_acc_set_mode(struct i2c_client *client, unsigned char mode)
{
	int err = -1;
    ...
    err = k2hh_acc_register_update(k2hh, (u8 *) data, K2HH_HR_REG, K2HH_HR_MSK, HR << K2HH_HR_POS);
    if (err < 0)
		return err;

	pr_info("k2hh_set_mode, mode : %d, LPen : %d, HR : %d\n", mode, LPen, HR);
	return 0;
}

static long k2hh_acc_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct k2hh_acc_data *acc = file->private_data;

	pr_info("%s: %s call with cmd 0x%x and arg 0x%x\n",
			K2HH_ACC_DEV_NAME, __func__, cmd, (unsigned int)arg);

	switch (cmd) {
	case K2HH_ACC_IOCTL_GET_SELFTEST:
	...
	case K2HH_ACC_IOCTL_GET_DELAY:
		interval = acc->pdata->poll_interval;
		if (copy_to_user(argp, &interval, sizeof(interval)))
			return -EFAULT;
		break;

	case K2HH_ACC_IOCTL_SET_DELAY:
	...
	case K2HH_ACC_IOCTL_SET_ENABLE:
		if (copy_from_user(&interval, argp, sizeof(interval)))
			return -EFAULT;
		if (interval > 1)
			return -EINVAL;
		if (interval)
			err = k2hh_acc_enable(acc);
		else
			err = k2hh_acc_disable(acc);
		return err;
		break;

	case K2HH_ACC_IOCTL_GET_ENABLE:
		interval = atomic_read(&acc->enabled);
		if (copy_to_user(argp, &interval, sizeof(interval)))
			return -EINVAL;
		break;

	case K2HH_ACC_IOCTL_SET_G_RANGE:
		if (copy_from_user(buf, argp, 1))
			return -EFAULT;
		bit_values = buf[0];
		err = k2hh_acc_update_g_range(acc, bit_values);
		if (err < 0)
			return err;
		break;

#ifdef INTERRUPT_MANAGEMENT
	case K2HH_ACC_IOCTL_SET_CTRL_REG3:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
		reg_address = CTRL_REG3;
		mask = buf[1];
		bit_values = buf[0];
		err = k2hh_acc_register_update(acc, (u8 *) arg, reg_address,
				mask, bit_values);
		if (err < 0)
			return err;
	...
	case K2HH_ACC_IOCTL_SET_CTRL_REG6:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_SET_DURATION1:
		if (copy_from_user(buf, argp, 1))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_SET_THRESHOLD1:
		if (copy_from_user(buf, argp, 1))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_SET_CONFIG1:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_GET_SOURCE1:
		err = k2hh_acc_register_read(acc, buf, INT_SRC1);
		if (err < 0)
			return err;
#if DEBUG
	...
	case K2HH_ACC_IOCTL_SET_DURATION2:
		if (copy_from_user(buf, argp, 1))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_SET_THRESHOLD2:
		if (copy_from_user(buf, argp, 1))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_SET_CONFIG2:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
	...
	case K2HH_ACC_IOCTL_GET_SOURCE2:
		err = k2hh_acc_register_read(acc, buf, INT_SRC2);
		if (err < 0)
			return err;
#if DEBUG
	...
	case K2HH_ACC_IOCTL_GET_TAP_SOURCE:
		err = k2hh_acc_register_read(acc, buf, TT_SRC);
		if (err < 0)
			return err;
#if DEBUG
		...
		if (copy_to_user(argp, buf, 1)) {
			dev_err(&acc->client->dev, "%s: %s error in copy_to_user\n",
					K2HH_ACC_DEV_NAME, __func__);
			return -EINVAL;
		}
		break;

	case K2HH_ACC_IOCTL_SET_TAP_CFG:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
		reg_address = TT_CFG;
		mask = buf[1];
	...
	case K2HH_ACC_IOCTL_SET_TAP_TLIM:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
		reg_address = TT_LIM;
		mask = buf[1];
	...
	case K2HH_ACC_IOCTL_SET_TAP_THS:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
		reg_address = TT_THS;
		mask = buf[1];
	...
	case K2HH_ACC_IOCTL_SET_TAP_TLAT:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
		reg_address = TT_TLAT;
		mask = buf[1];
	...
	case K2HH_ACC_IOCTL_SET_TAP_TW:
		if (copy_from_user(buf, argp, 2))
			return -EFAULT;
		reg_address = TT_TW;
		mask = buf[1];
...
static const struct file_operations k2hh_acc_misc_fops = {
		.owner = THIS_MODULE,
		.open = k2hh_acc_misc_open,
		.unlocked_ioctl = k2hh_acc_misc_ioctl,
};

static struct miscdevice k2hh_acc_misc_device = {
		.minor = MISC_DYNAMIC_MINOR,
		.name = K2HH_ACC_DEV_NAME,
		.fops = &k2hh_acc_misc_fops,
};

static void k2hh_acc_input_work_func(struct work_struct *work)
{
	struct k2hh_acc_data *acc;
	...
	acc = container_of((struct delayed_work *)work,
			struct k2hh_acc_data,	input_work);
...
{
	int err;
	INIT_DELAYED_WORK(&acc->input_work, k2hh_acc_input_work_func);
...
static int k2hh_pinctrl_init(struct k2hh_acc_data *data)
{
	int retval;
	...
	data->k2hh_pinctrl = devm_pinctrl_get(&(data->client->dev));
	if (IS_ERR_OR_NULL(data->k2hh_pinctrl)) {
		dev_dbg(&data->client->dev,
			"Target does not use pinctrl\n");
		retval = PTR_ERR(data->k2hh_pinctrl);
		data->k2hh_pinctrl = NULL;
		return retval;
	}
	data->gpio_state_active
		= pinctrl_lookup_state(data->k2hh_pinctrl, "k2hh_int_active");
	...
	data->gpio_state_suspend
		= pinctrl_lookup_state(data->k2hh_pinctrl, "k2hh_int_suspend");
...
static int k2hh_pinctrl_select(struct k2hh_acc_data *data, bool on)
{
	struct pinctrl_state *pins_state;
	int ret;

	dev_err(&data->client->dev,"k2hh_pinctrl_select call\n");
	pins_state = on ? data->gpio_state_active : data->gpio_state_suspend;
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(data->k2hh_pinctrl, pins_state);
		if (ret) {
			dev_err(&data->client->dev,
				"can not set %s pins\n",
				on ? "k2hh_active" : "k2hh_suspend");
			return ret
;		}
	} else
		dev_err(&data->client->dev,
	...
	do{
		...
		if((abs(acc_cal[K2HH_AXIS_X] - acc_cal_pre[K2HH_AXIS_X]) > K2HH_SHAKING_DETECT_THRESHOLD)
			...
			|| (abs((acc_cal[K2HH_AXIS_Z] - acc_cal_pre[K2HH_AXIS_Z])) > K2HH_SHAKING_DETECT_THRESHOLD)){
				atomic_set(&k2hh->fast_calib_rslt, 0);
	...
	if ((abs(sum[K2HH_AXIS_X]/CALIBRATION_DATA_AMOUNT) >TESTLIMIT_XY) ||
		(abs(sum[K2HH_AXIS_Y]/CALIBRATION_DATA_AMOUNT) >TESTLIMIT_XY) ||
		 ((abs(sum[K2HH_AXIS_Z]/CALIBRATION_DATA_AMOUNT) > TESTLIMIT_Z_USL_LSB) || (abs(sum[K2HH_AXIS_Z]/CALIBRATION_DATA_AMOUNT) < TESTLIMIT_Z_LSL_LSB))) {
			...
			atomic_set(&k2hh->fast_calib_rslt, 0);
			if (k2hh_store_reserved_reg(dev) < 0)
...
static ssize_t k2hh_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);

	return sprintf(buf, "%d\n", atomic_read(&k2hh->enabled));
}


static ssize_t k2hh_enable_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long onoff;
	int error;
    struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
	...
	if (onoff == 0)
		k2hh_acc_disable(k2hh);
	else if (onoff == 1)
		k2hh_acc_enable(k2hh);

	return size;
}

/* See CTRL_REG1(20h) register ODR<3:0> data rate configuration table at datasheet*/
static ssize_t k2hh_delay_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned char delay = 0;
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);

	if (k2hh_acc_get_odr(k2hh, &delay) < 0)
...
static ssize_t k2hh_delay_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (error)
		return error;
	if (k2hh_acc_update_odr(k2hh,data) < 0)
		pr_info("invalid content '%s', length = %d\n",buf,size);

	return size;
}

static ssize_t k2hh_sensor_cal_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int xyz[3] = { 0, };
	int err = 0;
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_sensor_raw_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int xyz[3] = { 0, };
	int err = 0;
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);

	err = k2hh_acc_get_acceleration_raw_data(k2hh,xyz);
	if (err < 0)
		return sprintf(buf, "Read Calibrated Data failed");
...
static ssize_t k2hh_opmode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned char data = 0;
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);

	if (k2hh_acc_get_mode(k2hh->client, &data) < 0) {
		return sprintf(buf,"Read error\n");
	} else {
		if (data == 0)
...
static ssize_t k2hh_opmode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long data = 0;
	int error = -1;
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
	...
	if (error)
		return error;
	if (k2hh_acc_set_mode(k2hh->client, (unsigned char) data) < 0)
		pr_info("invalid content '%s', length = %d\n",buf,size);

	return size;
}

static ssize_t k2hh_selftest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_selftest_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long data = 0;
	int error = -1;
	u8 buf_error[50];
...
static ssize_t k2hh_fast_calibration_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_fast_calibration_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
	unsigned int timeout_shaking = 0;
	unsigned long data;
...
static ssize_t k2hh_eeprom_writing_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_eeprom_writing_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int error = 0;
	error = k2hh_store_Calibration_data(dev);
	if (error)
...
static ssize_t k2hh_fast_calibration_x_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_fast_calibration_x_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	int error = 0;
	char axis = 'x';
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_fast_calibration_y_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_fast_calibration_y_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	int error = 0;
	char axis = 'y';
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);

	atomic_set(&k2hh->fast_calib_y_rslt, 0);
...
static ssize_t k2hh_fast_calibration_z_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);
...
static ssize_t k2hh_fast_calibration_z_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	int error = 0;
	char axis = 'z';
	struct k2hh_acc_data *k2hh = dev_get_drvdata(dev);

	atomic_set(&k2hh->fast_calib_z_rslt, 0);
...
static DEVICE_ATTR(enable, S_IRUGO|S_IWUSR |S_IWGRP, k2hh_enable_show, k2hh_enable_store);
static DEVICE_ATTR(poll_delay, S_IRUGO|S_IWUSR |S_IWGRP, k2hh_delay_show, k2hh_delay_store);
static DEVICE_ATTR(value, S_IRUGO|S_IWUSR|S_IWGRP, k2hh_sensor_cal_data_show, NULL);
static DEVICE_ATTR(raw, S_IRUGO|S_IWUSR|S_IWGRP, k2hh_sensor_raw_data_show, NULL);
static DEVICE_ATTR(opmode, S_IRUGO|S_IWUSR |S_IWGRP, k2hh_opmode_show, k2hh_opmode_store);
static DEVICE_ATTR(selftest, S_IRUGO|S_IWUSR |S_IWGRP, k2hh_selftest_show, k2hh_selftest_store);
static DEVICE_ATTR(run_fast_calibration, S_IRUGO|S_IWUSR|S_IWGRP,	k2hh_fast_calibration_show, k2hh_fast_calibration_store);
static DEVICE_ATTR(run_calibration, S_IRUGO|S_IWUSR|S_IWGRP,	k2hh_eeprom_writing_show, k2hh_eeprom_writing_store);
static DEVICE_ATTR(fast_calibration_x, S_IRUGO|S_IWUSR|S_IWGRP, k2hh_fast_calibration_x_show, k2hh_fast_calibration_x_store);
static DEVICE_ATTR(fast_calibration_y, S_IRUGO|S_IWUSR|S_IWGRP, k2hh_fast_calibration_y_show, k2hh_fast_calibration_y_store);
static DEVICE_ATTR(fast_calibration_z, S_IRUGO|S_IWUSR|S_IWGRP, k2hh_fast_calibration_z_show, k2hh_fast_calibration_z_store);
		...
		const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, " get_adapter num = %d",client->adapter->nr);
	...
	acc = devm_kzalloc(&client->dev, sizeof(struct k2hh_acc_data), GFP_KERNEL);
	if (acc == NULL) {
		err = -ENOMEM;
		dev_err(&client->dev,
	...
	pdata = devm_kzalloc(&client->dev, sizeof(struct k2hh_acc_platform_data), GFP_KERNEL);
	...
	pdata->power_on = k2hh_power_on;
	pdata->power_off = k2hh_power_off;

#ifndef USE_DEFAULT_PINCTRL_NAME
	err = k2hh_pinctrl_init(acc);
	if (err)
		pr_info("k2hh_pinctrl_init ERROR  = %d\n", err);

	if (!err && acc->k2hh_pinctrl) {
		err = k2hh_pinctrl_select(acc, true);
		if (err < 0) {
			pr_info("k2hh_pinctrl_select ERROR  = %d\n", err);
			goto exit_alloc_data_failed;
		}
	}
#endif
	...
	acc->irq1_work_queue = create_singlethread_workqueue("k2hh_acc_wq1");
	...
	acc->irq2_work_queue = create_singlethread_workqueue("k2hh_acc_wq2");
	...
	if (acc->pdata->gpio_int1 >= 0) {
		...
		pr_info("%s: %s has set irq1 to irq: %d mapped on gpio:%d\n",
			K2HH_ACC_DEV_NAME, __func__, acc->irq1, acc->pdata->gpio_int1);
	}
/* lgp-s19 not used interrupt2 */
	...
	if (acc->pdata->gpio_int2 >= 0) {
		...
		pr_info("%s: %s has set irq2 to irq: %d mapped on gpio:%d\n",
			K2HH_ACC_DEV_NAME, __func__, acc->irq2, acc->pdata->gpio_int2);

	}
#endif

	acc->resume_state[RES_CTRL_REG1] = (K2HH_ACC_ENABLE_ALL_AXES | K2HH_HR_MSK);
	acc->resume_state[RES_CTRL_REG2] = 0x00;
	acc->resume_state[RES_CTRL_REG3] = 0x00;
	acc->resume_state[RES_CTRL_REG4] = IF_ADD_INC;
	...
	err = k2hh_acc_update_g_range(acc, acc->pdata->g_range);
	if (err < 0) {
	...
	k2hh_acc_misc_data = acc;

	err = misc_register(&k2hh_acc_misc_device);
	if (err < 0) {
		dev_err(&client->dev,
				"misc K2HH_ACC_DEV_NAME register failed\n");
		goto err_input_cleanup;
	}
...
/* for test */
	...
	if (acc->pdata->gpio_int1 >= 0) {
		...
		err = request_irq(acc->irq1, k2hh_acc_isr1, IRQF_TRIGGER_RISING,
				"k2hh_acc_irq1", acc);
		if (err < 0) {
...
#ifdef USE_ACC_IRQ2
	if (acc->pdata->gpio_int2 >= 0) {
		err = request_irq(acc->irq2, k2hh_acc_isr2, IRQF_TRIGGER_RISING,
				"k2hh_acc_irq2", acc);
		if (err < 0) {
...
#endif
err_misc_dereg:
	misc_deregister(&k2hh_acc_misc_device);
err_input_cleanup:
...
#if defined(CONFIG_SENSOR_AUTO_DETECT_ACC)
int k2hh_local_init(void)
{
	pr_info("%s accelerometer driver: local init\n", K2HH_ACC_DEV_NAME);
	return i2c_add_driver(&k2hh_acc_driver);
}

int k2hh_local_remove(void)
{
...
#if defined(CONFIG_SENSOR_AUTO_DETECT_ACC)
	pr_info("%s accelerometer k2hh_local_init\n", K2HH_ACC_DEV_NAME);
	lge_sensor_acc_add(&k2hh_init_info);
	return 0;
#else
	pr_info("%s accelerometer k2hh_acc_init\n", K2HH_ACC_DEV_NAME);
	return i2c_add_driver(&k2hh_acc_driver);
#endif
}
