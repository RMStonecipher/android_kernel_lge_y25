...
#define KEY_WAKE_GPIO_NEW_PIN 52
#define KEY_GPIO_OLD_PIN 115
int isNewHw = 0;
int isSuspended = 0;
int suspendpressCode = 0;
int suspendpressState = 0;
#endif
...
#ifdef CONFIG_MACH_MSM8926_VFP_KR
static ssize_t pp2106_newhw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len;
...
static struct device_attribute pp2106_newhw_attr = __ATTR(newhw, S_IRUGO | S_IWUSR, pp2106_newhw_show, NULL);
#endif
...
static int pp2106_resume(struct platform_device *pdev)
{
	...
	pp2106_hwreset();
#ifdef CONFIG_MACH_MSM8926_VFP_KR
	if(suspendpressState == 1)
	{
#if defined(CONFIG_HALLIC_S5712ACDL1)
		if ((hall_ic_on == 0) && suspendpressCode) {
			input_report_key(pp2106_kbd_dev, suspendpressCode, 0);
			input_sync(pp2106_kbd_dev);
			pr_err("%s:release key scancode = %d\n", __func__,suspendpressCode);
		}
#else
		if (suspendpressCode) {
			input_report_key(pp2106_kbd_dev, suspendpressCode, 0);
...
#ifdef CONFIG_MACH_MSM8926_VFP_KR
	if ( gpio_get_value(KEY_GPIO_OLD_PIN) == 1 )
	{
		rc = gpio_tlmm_config(GPIO_CFG(KEY_WAKE_GPIO_NEW_PIN , 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
			if (rc)
				pr_info("Failed to gpio_tlmm_config 52: %d\n", rc );

		rc = gpio_tlmm_config(GPIO_CFG(KEY_GPIO_OLD_PIN, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
			if (rc)
				pr_info("Failed to gpio_tlmm_config 115 input pull up: %d\n", rc );
	}
	else if ( gpio_get_value(KEY_GPIO_OLD_PIN) == 0 )
	{
		rc = gpio_tlmm_config(GPIO_CFG(KEY_GPIO_OLD_PIN, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
			if (rc)
				pr_info("Failed to gpio_tlmm_config 115: %d\n", rc );

		pp2106_pdata->irq_pin = KEY_WAKE_GPIO_NEW_PIN ;
		isNewHw = 1;
	}
#endif
...
#ifdef CONFIG_MACH_MSM8926_VFP_KR
	device_create_file(&pdev->dev, &pp2106_newhw_attr);
#endif
...
#ifdef CONFIG_MACH_MSM8926_VFP_KR
/* debug log to  check GPIO level*/
	if ( gpio_get_value(KEY_GPIO_OLD_PIN) == 1 )
		pr_info("gpio 115 value is high\n");
	else if ( gpio_get_value(KEY_GPIO_OLD_PIN) == 0 )
		pr_info("gpio 115 value is low\n");

	if ( gpio_get_value(KEY_WAKE_GPIO_NEW_PIN) == 1 )
		pr_info("gpio 52 value is high\n");
	else if ( gpio_get_value(KEY_WAKE_GPIO_NEW_PIN) == 0 )
		pr_info("gpio 52 value is low\n");

	if ( gpio_get_value(KEY_GPIO_OLD_PIN) == 0 )
	{
		rc = enable_irq_wake(gpio_to_irq(pp2106_pdata->irq_pin));
			if (rc)
				pr_info("Failed to mark GPIO %d as wake source: %d\n", pp2106_pdata->irq_pin, rc );
	}
#endif
