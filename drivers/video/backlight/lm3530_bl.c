...
#if defined(CONFIG_LGE_LCD_BACKLIGHT_DEBUG)
#define lm3530_print_log pr_info
#else
#define lm3530_print_log pr_debug
#endif
...
static void lm3530_set_main_current_level(struct i2c_client *client, int level)
{
	...
	if (level != 0) {
		...
		lm3530_print_log( "%s() :level is : %d, cal_value is : %d\n", __func__, level, cal_value);
	}
	else {
...
void lm3530_backlight_on(int level)
{
	if (backlight_status == BL_OFF) {
		msleep(17);
#if defined(CONFIG_MACH_MSM8X10_Y25_TRF_US) || defined(CONFIG_MACH_MSM8X10_Y25C_TRF_US)
		printk("\n%s, ++ lm3530_backlight_on  \n",__func__);
#else
		...
		const struct i2c_device_id *id)
{
	...
	if(lge_get_bootreason() == 0x77665560) {
		pr_info("lcd dimming reboot reason: change default boot brightness\n");
		/* During booting up, backlight setting command is called with this value */
		bl_dev->props.brightness = pdata->min_brightness;
	} else {
