	...
	if (enable) {
		...
		if (!has_dsv_f) //          
		{
			mdss_dsi_panel_reset(pdata, 0);
				...
				if(gpio_is_valid(ctrl_pdata->resx2_gpio))
				{
					gpio_set_value(ctrl_pdata->resx2_gpio, 0);
					pr_err("RESX Off(resx2_gpio). \n");
					udelay(50);
				}
				else
				{
					pr_err("%s:%d, resx2_gpio gpio not specified\n",
									__func__, __LINE__);
				}
				pr_info("%s: RESX2 Low\n", __func__);

                }
                #endif //CONFIG_MACH_MSM8926_AKA_KR
		/*Disable DSV (VDD, VEE)*/
...
#elif defined CONFIG_LGE_MIPI_DSI_LGD_NT35521_WXGA	//E7Wifi & E7LTE
{
...
#if defined(CONFIG_MACH_MSM8226_E8WIFI) || defined(CONFIG_MACH_MSM8926_E8LTE) || defined(CONFIG_MACH_MSM8926_E7LTE_ATT_US) || defined(CONFIG_MACH_MSM8926_E7LTE_VZW_US) || defined(CONFIG_MACH_MSM8926_E7LTE_USC_US) || defined(CONFIG_MACH_MSM8926_E8LTE_KR)
...
#endif
	{
		...
		mdss_dsi_lane_config(pdata, 0);
					...
					if(gpio_is_valid(ctrl_pdata->resx2_gpio))
					{
						gpio_set_value((ctrl_pdata->resx2_gpio), 1);
						pr_err("RESX2 ON(resx2_gpio). \n");
						udelay(50);
					}
					else
					{
...
#if defined ( CONFIG_FB_MSM_MIPI_JDI_R69338_VIDEO_HD_PANEL)
struct mdss_dsi_ctrl_pdata *ctrl_pdata_for_touch = NULL;
#endif
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
...
#if defined (CONFIG_MACH_MSM8926_AKA_KR)
        if(lge_get_board_revno() == HW_REV_A){
		ctrl_pdata->resx2_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,resx2_gpio", 0);
		pr_err("%s	resx2_gpio : %d \n", __func__, ctrl_pdata->resx2_gpio);
		if (!gpio_is_valid(ctrl_pdata->resx2_gpio)) {
			pr_err("%s:%d, resx2_gpio gpio not specified\n",
							__func__, __LINE__);
		} else {
			rc = gpio_request(ctrl_pdata->resx2_gpio, "resx2_gpio");
			if (rc) {
				pr_err("request resx2_gpio failed, rc=%d\n",
					   rc);
				gpio_free(ctrl_pdata->resx2_gpio);
				return -ENODEV;
			}
			pr_err("request resx2_gpio successed\n");
			gpio_direction_output(ctrl_pdata->resx2_gpio, 1);
		}
        }
#endif
#if defined(CONFIG_MACH_MSM8926_JAGNM_ATT) || defined(CONFIG_MACH_MSM8926_JAGC_SPR) || defined(CONFIG_MACH_MSM8926_JAGNM_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGNM_KDDI_JP) || defined(CONFIG_MACH_MSM8926_JAGN_KR) || defined(CONFIG_MACH_MSM8926_JAGNM_RGS) || defined(CONFIG_MACH_MSM8926_JAGNM_VTR)
	if(lge_get_board_revno() >= HW_REV_B)
	...
	if (hw_rev <= HW_REV_B) {
		...
		if (!gpio_is_valid(ctrl_pdata->disp_fd_gpio)) {
			...
			gpio_set_value((ctrl_pdata->disp_fd_gpio), 1);
			pr_info("%s: fd(%d) on\n", __func__, ctrl_pdata->disp_fd_gpio);
		}
	}
#endif
