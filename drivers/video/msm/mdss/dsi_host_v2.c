...
#if defined(CONFIG_MACH_MSM8X10_W5) || defined(CONFIG_MACH_MSM8X10_W65) || defined(CONFIG_MACH_MSM8X10_Y30)
	printk("%s:%d: lcd id :lge_lcd_id : %d \n", __func__, __LINE__,lge_lcd_id);

	if (lge_lcd_id == 0)//primary ID
	{
		printk("%s:%d: Primary panel \n", __func__, __LINE__);
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node, "qcom,dsi-pref-prim-pan", 0);
	}
	else //for safe secondary
	{
		printk("%s:%d: Secondary panel \n", __func__, __LINE__);
		dsi_pan_node = of_parse_phandle(pdev->dev.of_node, "qcom,dsi-pref-secondary-pan", 0);
	}
#else   //qct original
