...
extern void mdss_dsi_bllp_enable(struct mdss_dsi_ctrl_pdata *ctrl, bool enable);
#endif

#if defined (CONFIG_MACH_MSM8X10_W5) || defined (CONFIG_MACH_MSM8X10_W6) || defined (CONFIG_MACH_MSM8X10_L70P) || defined (CONFIG_MACH_MSM8X10_Y25_TRF_US) || defined (CONFIG_MACH_MSM8X10_Y25C_TRF_US) || defined(CONFIG_MACH_MSM8X10_Y30)
...
void mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	!defined(CONFIG_LGE_MIPI_LGD_LG4591_VIDEO_HD_PANEL)
		if (gpio_is_valid(ctrl_pdata->disp_en_gpio))
...
//		}
...
#endif
#elif defined(CONFIG_LGE_MIPI_LGD_LG4591_VIDEO_HD_PANEL)
	mdelay(20);
	if (ctrl->on_cmds.cmd_cnt)
		mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds);
	...
	if (ctrl->on_cmds2.cmd_cnt)
		mdss_dsi_panel_cmds_send(ctrl, &ctrl->on_cmds2);
#else
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
...
#endif
    {
		...
		mdss_dsi_bllp_enable(ctrl, 1);
		if (ctrl->off_cmds.cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds);
		mdss_dsi_bllp_enable(ctrl, 0);
		...
		mdss_dsi_bllp_enable(ctrl, 1);
		if (ctrl->off_cmds2.cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds2);
		mdss_dsi_bllp_enable(ctrl, 0);
#else
		if (ctrl->off_cmds.cmd_cnt)								// QCT orignal
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds);
#endif
	}
#endif
...
#if defined(CONFIG_LGE_MIPI_LGD_LG4591_VIDEO_HD_PANEL)
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds2,
		"qcom,mdss-dsi-on-command2", "qcom,mdss-dsi-on-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds2,
