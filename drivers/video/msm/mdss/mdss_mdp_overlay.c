...
#if defined (CONFIG_LGE_MIPI_LGD_LG4591_VIDEO_HD_PANEL)
	ret = mdss_mdp_overlay_get_fb_pipe(mfd, &pipe, MDSS_MDP_MIXER_MUX_LEFT, req);
#else
...
static void load_bgr888rle_image(struct fb_info *fbi,
								char *image,
								int image_width,
								int image_height)
{
	int wcount = 0;
	int stride = fbi->var.xres - image_width;
	int image_len = image_width * image_height;

	struct bgr_data *dst = (struct bgr_data *)fbi->screen_base;

	if (image_len <= 0 || image_len > fbi->var.xres * fbi->var.yres)
	{
		pr_err("%s: Invalid input parameter, image_len = %d, fbi->var.xres * fbi->var.yres = %d\n",
												__func__, image_len, fbi->var.xres * fbi->var.yres);
		return;
	}
											...
											int *pipe_ndx, int splash_event)
{
	...
	if (splash_event == MDP_CREATE_SPLASH_OV) {
		...
		load_bgr888rle_image(fbi, splash_bgr888_image, SPLASH_IMAGE_WIDTH, SPLASH_IMAGE_HEIGHT);
