...
#ifdef CUSTOMER_HW10
#define SCAN_EBUSY_RETRY_LIMIT_MAX_CNT 60
#endif
	if (err == -EBUSY) {
#ifdef CUSTOMER_HW10
	    busy_count_max++;
...
#ifdef CUSTOMER_HW10
	if (busy_count_max > SCAN_EBUSY_RETRY_LIMIT_MAX_CNT) {
	    WL_ERR(("wo0ngs: too many EBUSY error, so sent HANG event to %s\n", ndev->name));
	    busy_count_max = 0;
	    net_os_send_hang_message(ndev);
	}
#endif
		...
		if (!wl_cfgp2p_is_p2p_action(frame, frame_len)) {
			...
			for_each_bss(bss_list, bi, i) {
				...
				if (wl_cfg80211_get_ioctl_version() == 1) {
					is5g = LCHSPEC_IS5G(bi->chanspec);
					channel = LCHSPEC_CHANNEL(bi->chanspec);
				} else {
					is5g = CHSPEC_IS5G(bi->chanspec);
					channel = CHSPEC_CHANNEL(bi->chanspec);
				}

				if (is5g && (bi->ctl_ch ? bi->ctl_ch : channel) == af_params->channel) {
...
#if defined(DUAL_ESCAN_RESULT_BUFFER) && defined(CUSTOMER_HW10)
	if( wl->escan_info.escan_buf[0] ) {
		WL_DBG(("J:Clear escan_buf[0] not to cache scanresults when scanabort.\n"));
		bzero(wl->escan_info.escan_buf[0], ESCAN_BUF_SIZE);
	}
	if( wl->escan_info.escan_buf[1] ) {
		WL_DBG(("J:Clear escan_buf[1] not to cache scanresults when scanabort.\n"));
		bzero(wl->escan_info.escan_buf[1], ESCAN_BUF_SIZE);
	}
#endif
