...
#if defined(CONFIG_LGE_DEBUG_MMC_PERF_INDEX)
#define ID_CNT 				10
...
unsigned int mmcqd_rq_count[ID_CNT]={0}, mmcqd_rd_rq_count[ID_CNT]={0};
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	{
		pr_info("%s:%s: MMC Queue Thread:%d, %d, %d, %d, %d \n", 
				mmc_hostname(card->host), __func__, mmcqd[0], mmcqd[1], mmcqd[2], mmcqd[3], mmcqd[4]);  
		mmccid_tag_t1 = time1;
	}
	...
	if(t_period >= (unsigned long long )PRT_TIME_PERIOD)
	{
		...
		{
			pr_info("%s:%s: mmcqd:%d Workload < 1%%, duty %lld, period %lld, req_cnt=%d \n", 
							mmc_hostname(card->host), __func__,
		...
		else
		{
			...
			pr_info("%s:%s: mmcqd:%d Workload=%d%%, duty %lld, period %lld00, req_cnt=%d \n", 
							mmc_hostname(card->host), __func__,
		...
		if (mmcqd_t_usage_wr[idx])
		{
			...
			if (mmcqd_t_usage_wr[idx])	// discard print if duration will <1ms
			{
				...
				pr_info("%s:%s: mmcqd:%d Write Throughput=%d kB/s, size: %d bytes, time:%lld ms\n", 
								mmc_hostname(card->host), __func__,
		...
		if (mmcqd_t_usage_rd[idx])
		{
			...
			if (mmcqd_t_usage_rd[idx])	// discard print if duration will <1ms
			{
				...
				pr_info("%s:%s:mmcqd:%d Read Throughput=%d kB/s, size: %d bytes, time:%lld ms\n", 
								mmc_hostname(card->host), __func__,
